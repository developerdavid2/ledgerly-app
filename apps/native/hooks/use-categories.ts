...
export function useCategories(type?: "income" | "expense") {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await apiClient.api.category.$get({
        query: type ? { type } : {},
      });
...
export function useCreateCategory() {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      const res = await apiClient.api.category.$post({ json: data });
...
export function useUpdateCategory() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const res = await apiClient.api.category[":id"].$put({
        param: { id },
        json: data,
      });
...
export function useDeleteCategory() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const res = await apiClient.api.category[":id"].$delete({
        param: { id },
      });
      if (!res.ok) {
